  ...
  if (isset($_SESSION['backend'])) {
    ...
    if ($_POST['id'] != "" AND $_POST['id'] != null AND $_POST['id'] != 0) {
      if (preg_match('/^[0-9]+$/', $_POST['id'])) {
            ...
            if ($q_1->rowCount() >= 1) {
              //==============================================================//
                $q_2 = dd_q('SELECT * FROM image_tb WHERE img_id = ?', [$_POST['id']]);
                if ($q_2->rowCount() >= 1) {
                  $row = $q_2->fetch(PDO::FETCH_ASSOC);
                  $q_3 = dd_q('DELETE FROM image_tb WHERE img_id = ?', [$_POST['id']]);
                  if ($q_3 == true) {
                    unlink("../img/product_image/{$row['img_name']}");
                    dd_return(true, "ลบรูปสำเร็จ", "{$row['img_pdid']}");
                  }
